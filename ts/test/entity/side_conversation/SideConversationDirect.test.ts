...
describe('SideConversationDirect', async () => {
  ...
  test('direct-list-side_conversation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-side_conversation', setup.live)) return
    if (skipIfMissingIds(t, setup, ["conversation01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_SIDE_CONVERSATION_ENTID']
