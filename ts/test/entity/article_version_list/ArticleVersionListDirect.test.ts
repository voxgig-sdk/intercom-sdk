...
describe('ArticleVersionListDirect', async () => {
  ...
  test('direct-list-article_version_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-article_version_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["article_version_list01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_ARTICLE_VERSION_LIST_ENTID']
