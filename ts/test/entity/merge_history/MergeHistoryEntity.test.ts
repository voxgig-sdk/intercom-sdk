...
describe('MergeHistoryEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let merge_history_ref01_data = Object.values(setup.data.existing.merge_history)[0] as any

    // LIST
    const merge_history_ref01_ent = client.MergeHistory()
    const merge_history_ref01_match: any = {}
    merge_history_ref01_match['contact_id'] = setup.idmap['contact01']

    const merge_history_ref01_list = (await merge_history_ref01_ent.list(merge_history_ref01_match)).map((e: any) => e.data())
