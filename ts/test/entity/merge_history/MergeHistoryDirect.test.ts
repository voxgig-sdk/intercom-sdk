...
describe('MergeHistoryDirect', async () => {
  ...
  test('direct-list-merge_history', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-merge_history', setup.live)) return
    if (skipIfMissingIds(t, setup, ["contact01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_MERGE_HISTORY_ENTID']
