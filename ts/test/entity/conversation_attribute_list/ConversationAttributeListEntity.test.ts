...
describe('ConversationAttributeListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'conversation_attribute_list.' + op, live)) return
    }
    ...
    let conversation_attribute_list_ref01_data = Object.values(setup.data.existing.conversation_attribute_list)[0] as any

    // LIST
    const conversation_attribute_list_ref01_ent = client.ConversationAttributeList()
    const conversation_attribute_list_ref01_match: any = {}

    const conversation_attribute_list_ref01_list = (await conversation_attribute_list_ref01_ent.list(conversation_attribute_list_ref01_match)).map((e: any) => e.data())
