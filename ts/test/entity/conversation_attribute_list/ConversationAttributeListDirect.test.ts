...
describe('ConversationAttributeListDirect', async () => {
  ...
  test('direct-list-conversation_attribute_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-conversation_attribute_list', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_CONVERSATION_ATTRIBUTE_LIST_ENTID']
