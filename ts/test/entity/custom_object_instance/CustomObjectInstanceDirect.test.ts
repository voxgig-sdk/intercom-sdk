...
describe('CustomObjectInstanceDirect', async () => {
  ...
  test('direct-load-custom_object_instance', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-custom_object_instance', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_CUSTOM_OBJECT_INSTANCE_ENTID']
