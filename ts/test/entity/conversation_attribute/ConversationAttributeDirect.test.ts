...
describe('ConversationAttributeDirect', async () => {
  ...
  test('direct-load-conversation_attribute', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-conversation_attribute', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_CONVERSATION_ATTRIBUTE_ENTID']
