...
describe('CompanyScrollEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'company_scroll.' + op, live)) return
    }
    ...
    let company_scroll_ref01_data = Object.values(setup.data.existing.company_scroll)[0] as any

    // LIST
    const company_scroll_ref01_ent = client.CompanyScroll()
    const company_scroll_ref01_match: any = {}

    const company_scroll_ref01_list = (await company_scroll_ref01_ent.list(company_scroll_ref01_match)).map((e: any) => e.data())
