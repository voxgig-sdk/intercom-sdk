...
describe('DataConnectorExecutionResultListDirect', async () => {
  ...
  test('direct-list-data_connector_execution_result_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-data_connector_execution_result_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["data_connector_execution_result_list01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_DATA_CONNECTOR_EXECUTION_RESULT_LIST_ENTID']
