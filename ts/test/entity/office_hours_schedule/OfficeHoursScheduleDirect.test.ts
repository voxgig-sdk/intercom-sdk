...
describe('OfficeHoursScheduleDirect', async () => {
  ...
  test('direct-load-office_hours_schedule', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-office_hours_schedule', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_OFFICE_HOURS_SCHEDULE_ENTID']
