...
describe('ConversationParticipantEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'remove']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'conversation_participant.' + op, live)) return
    }
    ...
    // REMOVE
    const conversation_participant_ref01_match_rm0: any = { id: conversation_participant_ref01_data.id }
