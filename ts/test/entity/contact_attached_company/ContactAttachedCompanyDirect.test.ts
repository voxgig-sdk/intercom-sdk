...
describe('ContactAttachedCompanyDirect', async () => {
  ...
  test('direct-list-contact_attached_company', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-contact_attached_company', setup.live)) return
    if (skipIfMissingIds(t, setup, ["contact_attached_company01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_CONTACT_ATTACHED_COMPANY_ENTID']
