...
describe('TicketStateEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'ticket_state.' + op, live)) return
    }
    ...
    let ticket_state_ref01_data = Object.values(setup.data.existing.ticket_state)[0] as any

    // LIST
    const ticket_state_ref01_ent = client.TicketState()
    const ticket_state_ref01_match: any = {}

    const ticket_state_ref01_list = (await ticket_state_ref01_ent.list(ticket_state_ref01_match)).map((e: any) => e.data())
