...
describe('DeletedInternalArticleObjectDirect', async () => {
  ...
  test('direct-list-deleted_internal_article_object', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-deleted_internal_article_object', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_DELETED_INTERNAL_ARTICLE_OBJECT_ENTID']
