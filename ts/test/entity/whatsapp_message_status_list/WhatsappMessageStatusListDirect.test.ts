...
describe('WhatsappMessageStatusListDirect', async () => {
  ...
  test('direct-list-whatsapp_message_status_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-whatsapp_message_status_list', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_WHATSAPP_MESSAGE_STATUS_LIST_ENTID']
