...
describe('PaginatedDirect', async () => {
  ...
  test('direct-list-paginated', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-paginated', setup.live)) return
    if (skipIfMissingIds(t, setup, ["newsfeed01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_PAGINATED_ENTID']
