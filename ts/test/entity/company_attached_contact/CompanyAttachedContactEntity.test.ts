...
describe('CompanyAttachedContactEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let company_attached_contact_ref01_data = Object.values(setup.data.existing.company_attached_contact)[0] as any

    // LIST
    const company_attached_contact_ref01_ent = client.CompanyAttachedContact()
    const company_attached_contact_ref01_match: any = {}
    company_attached_contact_ref01_match['company_id'] = setup.idmap['company01']

    const company_attached_contact_ref01_list = (await company_attached_contact_ref01_ent.list(company_attached_contact_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/company_attached_contact/CompanyAttachedContactTestData.json')
  ...
  let idmap = transform(
    ['company_attached_contact01','company_attached_contact02','company_attached_contact03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
