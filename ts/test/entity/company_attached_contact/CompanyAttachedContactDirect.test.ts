...
describe('CompanyAttachedContactDirect', async () => {
  ...
  test('direct-list-company_attached_contact', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-company_attached_contact', setup.live)) return
    if (skipIfMissingIds(t, setup, ["company_attached_contact01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_COMPANY_ATTACHED_CONTACT_ENTID']
