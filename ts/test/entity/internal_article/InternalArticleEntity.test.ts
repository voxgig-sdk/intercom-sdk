...
describe('InternalArticleEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['update', 'load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'internal_article.' + op, live)) return
    }
    ...
    let internal_article_ref01_data = Object.values(setup.data.existing.internal_article)[0] as any

    // UPDATE
    const internal_article_ref01_ent = client.InternalArticle()
    const internal_article_ref01_data_up0: any = {}
    ...
    ;(internal_article_ref01_data_up0 as any)[internal_article_ref01_markdef_up0.name] = internal_article_ref01_markdef_up0.value
    ...
    assert((internal_article_ref01_resdata_up0 as any)[internal_article_ref01_markdef_up0.name] === internal_article_ref01_markdef_up0.value)


    // LOAD
    const internal_article_ref01_match_dt0: any = {}
