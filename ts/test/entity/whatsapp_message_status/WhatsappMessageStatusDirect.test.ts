...
describe('WhatsappMessageStatusDirect', async () => {
  ...
  test('direct-load-whatsapp_message_status', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-whatsapp_message_status', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_WHATSAPP_MESSAGE_STATUS_ENTID']
