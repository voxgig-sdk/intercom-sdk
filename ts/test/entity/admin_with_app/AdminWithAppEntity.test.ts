...
describe('AdminWithAppEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'admin_with_app.' + op, live)) return
    }
    ...
    let admin_with_app_ref01_data = Object.values(setup.data.existing.admin_with_app)[0] as any

    // LIST
    const admin_with_app_ref01_ent = client.AdminWithApp()
    const admin_with_app_ref01_match: any = {}

    const admin_with_app_ref01_list = (await admin_with_app_ref01_ent.list(admin_with_app_ref01_match)).map((e: any) => e.data())
