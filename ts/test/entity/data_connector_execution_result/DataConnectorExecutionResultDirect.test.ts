...
describe('DataConnectorExecutionResultDirect', async () => {
  ...
  test('direct-load-data_connector_execution_result', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-data_connector_execution_result', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_DATA_CONNECTOR_EXECUTION_RESULT_ENTID']
