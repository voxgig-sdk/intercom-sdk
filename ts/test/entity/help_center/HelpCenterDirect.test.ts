...
describe('HelpCenterDirect', async () => {
  ...
  test('direct-load-help_center', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-help_center', setup.live)) return
  ...
  test('direct-list-help_center', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-help_center', setup.live)) return
    if (skipIfMissingIds(t, setup, ["help_center01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_HELP_CENTER_ENTID']
