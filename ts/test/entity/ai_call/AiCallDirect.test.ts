...
describe('AiCallDirect', async () => {
  ...
  test('direct-load-ai_call', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ai_call', setup.live)) return
    if (skipIfMissingIds(t, setup, ["conversation_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_AI_CALL_ENTID']
