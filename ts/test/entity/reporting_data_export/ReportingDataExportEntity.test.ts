...
describe('ReportingDataExportEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'reporting_data_export.' + op, live)) return
    }
    ...
    // LIST
    const reporting_data_export_ref01_match: any = {}

    const reporting_data_export_ref01_list = (await reporting_data_export_ref01_ent.list(reporting_data_export_ref01_match)).map((e: any) => e.data())
