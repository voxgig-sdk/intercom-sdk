...
describe('TeamMetricListDirect', async () => {
  ...
  test('direct-list-team_metric_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-team_metric_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["team_metric_list01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INTERCOM_TEST_TEAM_METRIC_LIST_ENTID']
