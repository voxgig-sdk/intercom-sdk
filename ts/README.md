...
`list()` resolves to an array of ActivityLog ENTITIES — every operation
...
| `metadata` | `Record<string, any>` | Additional data provided about Admin activity. |
| `performed_by` | `Record<string, any>` | Details about the Admin involved in the activity. |
...
| `role` | `Record<string, any>` | The role assigned to this admin. |
| `team_ids` | `any[]` | This object represents the avatar associated with the admin. |
| `team_priority_level` | `Record<string, any>` | Admin priority levels for teams |
| `type` | `string` | String representing the object's type. |
...
| Field | Type | Description |
| --- | --- | --- |
| `app` | `Record<string, any>` | App that the admin belongs to. |
| `avatar` | `Record<string, any>` | This object represents the avatar associated with the admin. |
| `away_mode_enabled` | `boolean` | Identifies if this admin is currently set in away mode. |
| `away_mode_reassign` | `boolean` | Identifies if this admin is set to automatically reassign new conversations to the apps default inbox. |
| `email` | `string` | The email of the admin. |
...
| `data` | `Record<string, any>` | Additional metadata about the call |
...
| `tags` | `Record<string, any>` | A list of tags objects associated with a conversation |
| `title` | `string` | The title of the article.For multilingual articles, this will be the title of the default language's content. |
| `translated_content` | `Record<string, any>` | The Translated Content of an Article. |
...
| Field | Type | Description |
| --- | --- | --- |
| `data` | `Record<string, any>` | An object containing the results of the search. |
| `pages` | `Record<string, any>` | Cursor-based pagination is a technique used in the Intercom API to navigate through large amounts of data. |
...
| Field | Type | Description |
| --- | --- | --- |
| `action` | `Record<string, any>` | The action a contact can take on the banner, or `null` when the banner has no action. |
...
| `custom_attributes` | `Record<string, any>` | The custom attributes you have set on the company. |
...
| `notes` | `Record<string, any>` | The list of notes associated with the company |
| `plan` | `Record<string, any>` | The name of the plan you have associated with the company. |
| `remote_created_at` | `number` | The time the company was created by you. |
| `segments` | `Record<string, any>` | The list of segments associated with the company |
| `session_count` | `number` | How many sessions the company has recorded. |
| `size` | `number` | The number of employees in the company. |
| `tags` | `Record<string, any>` | The list of tags associated with the company |
...
| `companies` | `Record<string, any>` | An object with metadata about companies attached to a contact . |
| `created_at` | `number` | (Unix timestamp in seconds) The time when the contact was created. |
| `custom_attributes` | `Record<string, any>` | The custom attributes which are set for the contact. |
...
| `location` | `Record<string, any>` | An object containing location meta data about a Intercom contact. |
...
| `notes` | `Record<string, any>` | An object containing notes meta data about the notes that a contact has. |
...
| `social_profiles` | `Record<string, any>` | An object containing social profiles that a contact has. |
| `tags` | `Record<string, any>` | An object containing tags meta data about the tags that a contact has. |
| `type` | `string` | The type of object. |
...
| `unsubscribed_from_emails` | `boolean` | Whether the contact is unsubscribed from emails. |
| `updated_at` | `number` | (Unix timestamp in seconds) The time when the contact was last updated. |
| `user` | `Record<string, any>` | The unique identifiers retained after converting or merging. |
| `visitor` | `Record<string, any>` | The unique identifiers to convert a single Visitor. |
| `workspace_id` | `string` | The id of the workspace which the contact belongs to. |
...
| `query` | `any` |  |
| `sort` | `Record<string, any>` | An optional object to sort the results by. |
...
| `pages` | `Record<string, any>` | Pagination metadata, including links to neighbouring pages. |
...
| `ai_agent` | `Record<string, any>` | Data related to AI Agent involvement in the conversation. |
...
| `channel` | `Record<string, any>` | The channel through which the conversation was initiated and its current channel. |
| `company` | `Record<string, any>` | The company associated with the conversation. |
| `company_id` | `string` | The ID of the company that the conversation is associated with. |
| `contacts` | `Record<string, any>` | The list of contacts (users or leads) involved in this conversation. |
...
| `conversation_parts` | `Record<string, any>` | A list of Conversation Part objects for each part message in the conversation. |
| `conversation_rating` | `Record<string, any>` | The Conversation Rating object which contains information on the rating and/or remark added by a Contact and the Admin assigned to the conversation. |
| `created_at` | `number` | The time the conversation was created. |
| `custom_attributes` | `Record<string, any>` | An object containing the different custom attributes associated to the conversation as key-value pairs. |
| `external_references` | `any[]` | References linking this conversation to records in an external helpdesk or CRM system. |
| `first_contact_reply` | `Record<string, any>` | An object containing information on the first users message. |
| `from` | `Record<string, any>` |  |
| `id` | `string` | The id representing the conversation. |
| `linked_objects` | `Record<string, any>` | An object containing metadata about linked conversations and linked tickets. |
...
| `sales_agent` | `Record<string, any>` | Data related to Sales Agent involvement in the conversation. |
...
| `sla_applied` | `Record<string, any>` | The SLA Applied object contains the details for which SLA has been applied to this conversation. |
| `snoozed_until` | `number` | If set this is the time in the future when this conversation will be marked as open. |
| `source` | `Record<string, any>` | The type of the conversation part that started this conversation. |
| `state` | `string` | Can be set to "open", "closed" or "snoozed". |
| `statistics` | `Record<string, any>` | A Statistics object containing all information required for reporting, with timestamps and calculated metrics. |
| `subject` | `string` | The title of the email. |
| `tags` | `Record<string, any>` | A list of tags objects associated with a conversation |
| `team_assignee_id` | `number` | The id of the team assigned to the conversation. |
| `teammates` | `Record<string, any>` | The list of teammates who participated in the conversation (wrote at least one conversation part). |
...
| `reference` | `Record<string, any>` | (Relationship data type only) Reference configuration for related objects. |
...
| --- | --- | --- |
| `created_at` | `number` |  |
| `custom_attributes` | `Record<string, any>` | The custom attributes which are set for the Custom Object instance. |
...
| `id` | `string` |  |
| `pages` | `Record<string, any>` | The majority of list resources in the API are paginated to allow clients to traverse data over multiple requests. |
...
| `mock_response` | `Record<string, any>` | A sample JSON response from the external API. |
...
| `event_summaries` | `Record<string, any>` | A list of event summaries for the user. |
| `id` | `string` | The unique identifier for the contact (lead or user) which is given by Intercom. |
| `metadata` | `Record<string, any>` | Optional metadata about the event. |
...
| `conversation` | `Record<string, any>` | Conversation-related attribute errors. |
| `conversation_id` | `string` | The external ID of the rated conversation. |
| `conversation_metadata` | `Record<string, any>` | Metadata about the conversation, including history and attributes. |
| `message` | `Record<string, any>` | A message exchanged within a Fin Agent conversation. |
...
| `user` | `Record<string, any>` | User-related attribute errors. |
...
| `teammate` | `Record<string, any>` | A reference to a teammate |
...
| Field | Type | Description |
| --- | --- | --- |
| `ar` | `Record<string, any>` | The content of the group in Arabic |
...
| `de` | `Record<string, any>` | The content of the group in German |
...
| `el` | `Record<string, any>` | The content of the group in Greek |
| `en` | `Record<string, any>` | The content of the group in English |
| `es` | `Record<string, any>` | The content of the group in Spanish |
| `et` | `Record<string, any>` | The content of the group in Estonian |
| `fi` | `Record<string, any>` | The content of the group in Finnish |
| `fr` | `Record<string, any>` | The content of the group in French |
| `from_url` | `string` | The source URL that is redirected. |
| `he` | `Record<string, any>` | The content of the group in Hebrew |
...
| `ko` | `Record<string, any>` | The content of the group in Korean |
| `locale` | `string` | The locale of the redirect's target. |
| `locales` | `any[]` | The locales in which the help center is available. |
| `lt` | `Record<string, any>` | The content of the group in Lithuanian |
| `lv` | `Record<string, any>` | The content of the group in Latvian |
| `mn` | `Record<string, any>` | The content of the group in Mongolian |
| `name` | `string` | The name of the collection. |
| `nb` | `Record<string, any>` | The content of the group in Norwegian |
| `nl` | `Record<string, any>` | The content of the group in Dutch |
| `parent_id` | `string` | The id of the parent collection. |
| `pl` | `Record<string, any>` | The content of the group in Polish |
| `pt` | `Record<string, any>` | The content of the group in Portuguese (Portugal) |
| `ptBR` | `Record<string, any>` | The content of the group in Portuguese (Brazil) |
| `ro` | `Record<string, any>` | The content of the group in Romanian |
| `ru` | `Record<string, any>` | The content of the group in Russian |
| `sl` | `Record<string, any>` | The content of the group in Slovenian |
| `sr` | `Record<string, any>` | The content of the group in Serbian |
| `sv` | `Record<string, any>` | The content of the group in Swedish |
...
| `zhCN` | `Record<string, any>` | The content of the group in Chinese (China) |
| `zhTW` | `Record<string, any>` | The content of the group in Chinese (Taiwan) |
...
| `created_at` | `number` | The time the conversation was created. |
| `from` | `Record<string, any>` | The sender of the message. |
...
| `author` | `Record<string, any>` | Optional. |
| `body` | `string` | The body text of the note. |
| `company` | `Record<string, any>` | Represents the company that the note was created about. |
| `contact` | `Record<string, any>` | Represents the contact that the note was created about. |
...
| `default_translation` | `Record<string, any>` | A translation object contains the localised details of a subscription type. |
...
| `applied_by` | `Record<string, any>` | The admin who applied the tag. |
| `companies` | `any[]` |  |
...
| `admin_priority_level` | `Record<string, any>` | Admin priority levels for the team |
...
| `attributes` | `Record<string, any>` | The attributes set on the ticket. |
| `category` | `string` | Category of the Ticket. |
| `contacts` | `Record<string, any>` | The list of contacts affected by a ticket. |
...
| `ticket_attributes` | `Record<string, any>` | An object containing the different attributes associated to the ticket as key-value pairs. |
| `ticket_id` | `string` | The ID of the Ticket used in the Intercom Inbox and Messenger. |
| `ticket_parts` | `Record<string, any>` | A list of Ticket Part objects for each note and event in the ticket. |
| `ticket_state` | `Record<string, any>` | A ticket state, used to define the state of a ticket. |
| `ticket_state_id` | `string` | The ID of the ticket state associated with the ticket type. |
| `ticket_type` | `Record<string, any>` | A ticket type, used to define the data fields to be captured in a ticket. |
...
| `author` | `Record<string, any>` | The author that wrote or triggered the part. |
| `body` | `string` | The message body, which may contain HTML. |
| `created_at` | `number` | The time the note was created. |
...
| `ticket_types` | `Record<string, any>` | A list of ticket types associated with a given ticket state. |
| `type` | `string` | String representing the object's type. |
...
| `ticket_states` | `Record<string, any>` | A list of ticket states associated with a given ticket type. |
| `ticket_type_attributes` | `Record<string, any>` | A list of attributes associated with a given ticket type. |
| `type` | `string` | String representing the object's type. |
...
| `custom_attributes` | `Record<string, any>` | The custom attributes you have set on the Visitor. |
...
| `snapshot` | `Record<string, any>` | The current snapshot of workflow steps and configuration. |
| `state` | `string` | The current state of the workflow. |
| `target_channels` | `any[]` | The channels this workflow targets. |
| `targeting` | `Record<string, any>` | The targeting rules for this workflow. |
