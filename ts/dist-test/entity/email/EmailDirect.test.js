...
(0, node_test_1.describe)('EmailDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-email', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-email', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['INTERCOM_TEST_EMAIL_ENTID'];
