...
func TestSubscriptionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set INTERCOM_TEST_SUBSCRIPTION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		subscriptionRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "subscription"}), "subscription_ref01"))
		subscriptionRef01Data["contact_id"] = setup.idmap["contact01"]
...
func subscriptionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"subscription01", "subscription02", "subscription03", "contact01", "contact02", "contact03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("INTERCOM_TEST_SUBSCRIPTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["INTERCOM_TEST_SUBSCRIPTION_ENTID"])
