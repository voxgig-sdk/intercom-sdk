...
func TestCallDirect(t *testing.T) {
	t.Run("direct-list-call", func(t *testing.T) {
		setup := callDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-call", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-call", func(t *testing.T) {
		setup := callDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-call", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type callDirectSetupResult struct {
...
func callDirectSetup(mockres any) *callDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"INTERCOM_TEST_CALL_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["INTERCOM_TEST_CALL_ENTID"]; ok {
		...
		return &callDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &callDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
