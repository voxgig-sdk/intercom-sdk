...
func TestTicketTypeDirect(t *testing.T) {
	t.Run("direct-list-ticket_type", func(t *testing.T) {
		setup := ticket_typeDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-ticket_type", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-ticket_type", func(t *testing.T) {
		setup := ticket_typeDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-ticket_type", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type ticket_typeDirectSetupResult struct {
...
func ticket_typeDirectSetup(mockres any) *ticket_typeDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"INTERCOM_TEST_TICKET_TYPE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["INTERCOM_TEST_TICKET_TYPE_ENTID"]; ok {
		...
		return &ticket_typeDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &ticket_typeDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
