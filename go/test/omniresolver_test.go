...
import (
	...
	omni "github.com/voxgig-sdk/intercom-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IntercomError); is {
				out := map[string]any{
					"name":    "IntercomError",
...
func omniCtx(arg any, client *sdk.IntercomSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IntercomSDK); is {
			client = live
