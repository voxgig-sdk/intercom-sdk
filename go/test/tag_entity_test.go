...
func TestTagEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set INTERCOM_TEST_TAG_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		tagRef01Data["article_id"] = setup.idmap["article01"]
		tagRef01Data["contact_id"] = setup.idmap["contact01"]
		tagRef01Data["content_snippet_id"] = setup.idmap["content_snippet01"]
		tagRef01Data["conversation_id"] = setup.idmap["conversation01"]
		tagRef01Data["internal_article_id"] = setup.idmap["internal_article01"]
		tagRef01Data["ticket_id"] = setup.idmap["ticket01"]
...
func tagBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"tag01", "tag02", "tag03", "article01", "article02", "article03", "contact01", "contact02", "contact03", "content_snippet01", "content_snippet02", "content_snippet03", "conversation01", "conversation02", "conversation03", "internal_article01", "internal_article02", "internal_article03", "ticket01", "ticket02", "ticket03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("INTERCOM_TEST_TAG_ENTID")
	...
	idmapResolved := core.ToMapAny(env["INTERCOM_TEST_TAG_ENTID"])
