...
func TestExternalPageDirect(t *testing.T) {
	t.Run("direct-list-external_page", func(t *testing.T) {
		setup := external_pageDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-external_page", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-external_page", func(t *testing.T) {
		setup := external_pageDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-external_page", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type external_pageDirectSetupResult struct {
...
func external_pageDirectSetup(mockres any) *external_pageDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"INTERCOM_TEST_EXTERNAL_PAGE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["INTERCOM_TEST_EXTERNAL_PAGE_ENTID"]; ok {
		...
		return &external_pageDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &external_pageDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
