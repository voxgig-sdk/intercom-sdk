...
func TestContentEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set INTERCOM_TEST_CONTENT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func contentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("INTERCOM_TEST_CONTENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["INTERCOM_TEST_CONTENT_ENTID"])
