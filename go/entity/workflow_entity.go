...
func NewWorkflowEntity(client *core.IntercomSDK, entopts map[string]any) *WorkflowEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *WorkflowEntity) GetName() string { return e.name }
...
func (e *WorkflowEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *WorkflowEntity) Deleted() bool {
	return e.deleted
}


func (e *WorkflowEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewWorkflowEntity(e.client, opts)
}

func (e *WorkflowEntity) Data(args ...any) any {
...
func (e *WorkflowEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Workflow; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *WorkflowEntity) DataTyped(data ...Workflow) Workflow {
	if len(data) > 0 {
		return typedFrom[Workflow](e.Data(asMap(data[0])))
	}
	return typedFrom[Workflow](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Workflow (all fields
// optional at the wire level).
func (e *WorkflowEntity) MatchTyped(match ...Workflow) Workflow {
	if len(match) > 0 {
		return typedFrom[Workflow](e.Match(asMap(match[0])))
	}
	return typedFrom[Workflow](e.Match())
}
...
func (e *WorkflowEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *WorkflowEntity) Load(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// WorkflowLoadMatch and returns an Workflow. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *WorkflowEntity) LoadTyped(reqmatch WorkflowLoadMatch, ctrl map[string]any) (Workflow, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return Workflow{}, err
	}
	return typedFrom[Workflow](res), nil
}



func (e *WorkflowEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *WorkflowEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *WorkflowEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *WorkflowEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *WorkflowEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
