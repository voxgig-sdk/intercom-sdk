...
func NewTicketReplyEntity(client *core.IntercomSDK, entopts map[string]any) *TicketReplyEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *TicketReplyEntity) GetName() string { return e.name }
...
func (e *TicketReplyEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *TicketReplyEntity) Deleted() bool {
	return e.deleted
}


func (e *TicketReplyEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewTicketReplyEntity(e.client, opts)
}

func (e *TicketReplyEntity) Data(args ...any) any {
...
func (e *TicketReplyEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an TicketReply; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *TicketReplyEntity) DataTyped(data ...TicketReply) TicketReply {
	if len(data) > 0 {
		return typedFrom[TicketReply](e.Data(asMap(data[0])))
	}
	return typedFrom[TicketReply](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through TicketReply (all fields
// optional at the wire level).
func (e *TicketReplyEntity) MatchTyped(match ...TicketReply) TicketReply {
	if len(match) > 0 {
		return typedFrom[TicketReply](e.Match(asMap(match[0])))
	}
	return typedFrom[TicketReply](e.Match())
}
...
func (e *TicketReplyEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *TicketReplyEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}


func (e *TicketReplyEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}



func (e *TicketReplyEntity) Create(reqdata map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// TicketReplyCreateData and returns an TicketReply. It delegates to the untyped
// Create (identical runtime) and converts at the typed boundary.
func (e *TicketReplyEntity) CreateTyped(reqdata TicketReplyCreateData, ctrl map[string]any) (TicketReply, error) {
	res, err := e.Create(asMap(reqdata), ctrl)
	if err != nil {
		return TicketReply{}, err
	}
	return typedFrom[TicketReply](res), nil
}



func (e *TicketReplyEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *TicketReplyEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *TicketReplyEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
