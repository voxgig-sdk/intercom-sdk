...
class ConversationAttributeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ConversationAttributeEntity(self._client, opts)
    ...
    def data_get(self) -> ConversationAttribute:
    ...
    def match_get(self) -> ConversationAttribute:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ConversationAttribute:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ConversationAttribute().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: ConversationAttributeCreateData, ctrl=None) -> ConversationAttribute:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: ConversationAttributeUpdateData, ctrl=None) -> ConversationAttribute:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ConversationAttribute:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ConversationAttribute().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
