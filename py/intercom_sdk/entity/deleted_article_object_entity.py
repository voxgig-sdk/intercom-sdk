...
class DeletedArticleObjectEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DeletedArticleObjectEntity(self._client, opts)
    ...
    def data_get(self) -> DeletedArticleObject:
    ...
    def match_get(self) -> DeletedArticleObject:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> DeletedArticleObject:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.DeletedArticleObject().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
