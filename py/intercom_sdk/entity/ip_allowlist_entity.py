...
class IpAllowlistEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return IpAllowlistEntity(self._client, opts)
    ...
    def match_get(self) -> IpAllowlist:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[IpAllowlist]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.IpAllowlist().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: IpAllowlistUpdateData, ctrl=None) -> IpAllowlist:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
