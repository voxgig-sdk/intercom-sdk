...
class InternalArticleEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return InternalArticleEntity(self._client, opts)
    ...
    def data_get(self) -> InternalArticle:
    ...
    def match_get(self) -> InternalArticle:
    ...
    def load(self, reqmatch=None, ctrl=None) -> InternalArticle:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.InternalArticle().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: InternalArticleUpdateData, ctrl=None) -> InternalArticle:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
