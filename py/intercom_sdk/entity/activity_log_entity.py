...
class ActivityLogEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ActivityLogEntity(self._client, opts)
    ...
    def match_get(self) -> ActivityLog:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ActivityLog]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ActivityLog().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
