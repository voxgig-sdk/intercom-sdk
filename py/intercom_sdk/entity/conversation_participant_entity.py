...
class ConversationParticipantEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ConversationParticipantEntity(self._client, opts)
    ...
    def data_get(self) -> ConversationParticipant:
    ...
    def match_get(self) -> ConversationParticipant:
    ...
    def create(self, reqdata: ConversationParticipantCreateData, ctrl=None) -> ConversationParticipant:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ConversationParticipant:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ConversationParticipant().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
