...
class NewsItemEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return NewsItemEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> NewsItem:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.NewsItem().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: NewsItemUpdateData, ctrl=None) -> NewsItem:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
