...
class TicketStateEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TicketStateEntity(self._client, opts)
    ...
    def match_get(self) -> TicketState:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[TicketState]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.TicketState().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
