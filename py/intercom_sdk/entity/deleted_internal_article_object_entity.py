...
class DeletedInternalArticleObjectEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DeletedInternalArticleObjectEntity(self._client, opts)
    ...
    def data_get(self) -> DeletedInternalArticleObject:
    ...
    def match_get(self) -> DeletedInternalArticleObject:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[DeletedInternalArticleObject]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.DeletedInternalArticleObject().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: DeletedInternalArticleObjectCreateData, ctrl=None) -> DeletedInternalArticleObject:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> DeletedInternalArticleObject:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.DeletedInternalArticleObject().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
