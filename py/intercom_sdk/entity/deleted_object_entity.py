...
class DeletedObjectEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DeletedObjectEntity(self._client, opts)
    ...
    def data_get(self) -> DeletedObject:
    ...
    def match_get(self) -> DeletedObject:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> DeletedObject:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.DeletedObject().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
