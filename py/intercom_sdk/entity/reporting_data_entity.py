...
class ReportingDataEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ReportingDataEntity(self._client, opts)
    ...
    def data_get(self) -> ReportingData:
    ...
    def match_get(self) -> ReportingData:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ReportingData:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ReportingData().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
