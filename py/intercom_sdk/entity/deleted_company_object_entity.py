...
class DeletedCompanyObjectEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DeletedCompanyObjectEntity(self._client, opts)
    ...
    def data_get(self) -> DeletedCompanyObject:
    ...
    def match_get(self) -> DeletedCompanyObject:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> DeletedCompanyObject:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.DeletedCompanyObject().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
