...
class DeletedDataConnectorObjectEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DeletedDataConnectorObjectEntity(self._client, opts)
    ...
    def data_get(self) -> DeletedDataConnectorObject:
    ...
    def match_get(self) -> DeletedDataConnectorObject:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> DeletedDataConnectorObject:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.DeletedDataConnectorObject().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
