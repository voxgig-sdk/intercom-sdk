...
class CustomObjectInstanceEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CustomObjectInstanceEntity(self._client, opts)
    ...
    def data_get(self) -> CustomObjectInstance:
    ...
    def match_get(self) -> CustomObjectInstance:
    ...
    def load(self, reqmatch=None, ctrl=None) -> CustomObjectInstance:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.CustomObjectInstance().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: CustomObjectInstanceCreateData, ctrl=None) -> CustomObjectInstance:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> CustomObjectInstance:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.CustomObjectInstance().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
