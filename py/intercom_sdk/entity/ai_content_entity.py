...
class AiContentEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AiContentEntity(self._client, opts)
    ...
    def remove(self, reqmatch=None, ctrl=None) -> AiContent:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.AiContent().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
