...
class TestTicketStateDirect:

    def test_should_direct_list_ticket_state(self):
        setup = _ticket_state_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-ticket_state", "live" if setup["live"] else "unit")
        if _skip:
...
def _ticket_state_direct_setup(mockres):
