...
class TestAdminWithAppDirect:

    def test_should_direct_list_admin_with_app(self):
        setup = _admin_with_app_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-admin_with_app", "live" if setup["live"] else "unit")
        if _skip:
...
def _admin_with_app_direct_setup(mockres):
