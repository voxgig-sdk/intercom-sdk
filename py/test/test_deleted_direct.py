...
class TestDeletedDirect:

    def test_should_direct_list_deleted(self):
        setup = _deleted_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-deleted", "live" if setup["live"] else "unit")
        if _skip:
...
def _deleted_direct_setup(mockres):
