...
class TestTicketDirect:

    def test_should_direct_load_ticket(self):
        setup = _ticket_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ticket", "live" if setup["live"] else "unit")
        if _skip:
...
def _ticket_direct_setup(mockres):
