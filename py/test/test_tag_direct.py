...
class TestTagDirect:
    ...
    def test_should_direct_list_tag(self):
        setup = _tag_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-tag", "live" if setup["live"] else "unit")
        if _skip:
