...
class TestCompanyScrollDirect:

    def test_should_direct_list_company_scroll(self):
        setup = _company_scroll_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-company_scroll", "live" if setup["live"] else "unit")
        if _skip:
...
def _company_scroll_direct_setup(mockres):
