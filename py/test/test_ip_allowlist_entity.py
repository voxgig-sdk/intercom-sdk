...
class TestIpAllowlistEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.IpAllowlist(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.IpAllowlist(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set INTERCOM_TEST_IP_ALLOWLIST_ENTID JSON to run live")
        client = setup["client"]
        ...
        ip_allowlist_ref01_list_result = ip_allowlist_ref01_ent.list(ip_allowlist_ref01_match, None)
        assert isinstance(ip_allowlist_ref01_list_result, list)
        ...
        ip_allowlist_ref01_resdata_up0 = helpers.to_map(runner.entity_data(ip_allowlist_ref01_ent.update(ip_allowlist_ref01_data_up0_up, None)))
...
def _ip_allowlist_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "INTERCOM_TEST_IP_ALLOWLIST_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("INTERCOM_TEST_IP_ALLOWLIST_ENTID"))
