    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set INTERCOM_TEST_DATA_ENTID JSON to run live")
        client = setup["client"]
        ...
        data_ref01_data = helpers.to_map(runner.entity_data(data_ref01_ent.create(data_ref01_data, None)))
        assert data_ref01_data is not None
        assert data_ref01_data["id"] is not None
        ...
        data_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(data_ref01_data_dt0_loaded))
        assert data_ref01_data_dt0_load_result is not None
        assert data_ref01_data_dt0_load_result["id"] == data_ref01_data["id"]
...
def _data_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("INTERCOM_TEST_DATA_ENTID"))
