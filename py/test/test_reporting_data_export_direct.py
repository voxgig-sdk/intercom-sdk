...
class TestReportingDataExportDirect:

    def test_should_direct_list_reporting_data_export(self):
        setup = _reporting_data_export_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-reporting_data_export", "live" if setup["live"] else "unit")
        if _skip:
...
def _reporting_data_export_direct_setup(mockres):
