...
class TestContentSearchDirect:

    def test_should_direct_list_content_search(self):
        setup = _content_search_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-content_search", "live" if setup["live"] else "unit")
        if _skip:
...
def _content_search_direct_setup(mockres):
