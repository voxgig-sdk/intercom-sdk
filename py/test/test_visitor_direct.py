...
class TestVisitorDirect:
    ...
    def test_should_direct_load_visitor(self):
        setup = _visitor_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-visitor", "live" if setup["live"] else "unit")
        if _skip:
...
def _visitor_direct_setup(mockres):
