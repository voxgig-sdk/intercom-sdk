...
class TestIpAllowlistDirect:
    ...
    def test_should_direct_list_ip_allowlist(self):
        setup = _ip_allowlist_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-ip_allowlist", "live" if setup["live"] else "unit")
        if _skip:
