...
class TestOfficeHourDirect:

    def test_should_direct_list_office_hour(self):
        setup = _office_hour_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-office_hour", "live" if setup["live"] else "unit")
        if _skip:
...
def _office_hour_direct_setup(mockres):
