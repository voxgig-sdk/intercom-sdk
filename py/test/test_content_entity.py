    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set INTERCOM_TEST_CONTENT_ENTID JSON to run live")
        client = setup["client"]
        ...
        content_ref01_data = helpers.to_map(runner.entity_data(content_ref01_ent.create(content_ref01_data, None)))
...
def _content_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("INTERCOM_TEST_CONTENT_ENTID"))
