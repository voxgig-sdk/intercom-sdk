...
class TestDataAttributeDirect:

    def test_should_direct_list_data_attribute(self):
        setup = _data_attribute_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-data_attribute", "live" if setup["live"] else "unit")
        if _skip:
...
def _data_attribute_direct_setup(mockres):
