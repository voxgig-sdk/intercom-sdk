...
class TestActivityLogDirect:

    def test_should_direct_list_activity_log(self):
        setup = _activity_log_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-activity_log", "live" if setup["live"] else "unit")
        if _skip:
...
def _activity_log_direct_setup(mockres):
