...
class TestTeamMetricListDirect:

    def test_should_direct_list_team_metric_list(self):
        setup = _team_metric_list_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-team_metric_list", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["team_metric_list01"]:
...
def _team_metric_list_direct_setup(mockres):
