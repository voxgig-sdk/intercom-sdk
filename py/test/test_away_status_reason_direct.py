...
class TestAwayStatusReasonDirect:

    def test_should_direct_list_away_status_reason(self):
        setup = _away_status_reason_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-away_status_reason", "live" if setup["live"] else "unit")
        if _skip:
...
def _away_status_reason_direct_setup(mockres):
