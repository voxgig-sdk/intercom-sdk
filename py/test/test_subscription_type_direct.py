...
class TestSubscriptionTypeDirect:

    def test_should_direct_list_subscription_type(self):
        setup = _subscription_type_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-subscription_type", "live" if setup["live"] else "unit")
        if _skip:
...
def _subscription_type_direct_setup(mockres):
