...
class TestNewsfeedDirect:

    def test_should_direct_load_newsfeed(self):
        setup = _newsfeed_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-newsfeed", "live" if setup["live"] else "unit")
        if _skip:
...
def _newsfeed_direct_setup(mockres):
