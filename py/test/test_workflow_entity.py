...
class TestWorkflowEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Workflow(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _workflow_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "workflow." + _op, "live" if _live else "unit")
            if _skip:
...
def _workflow_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/workflow/WorkflowTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("INTERCOM_TEST_WORKFLOW_ENTID"))
