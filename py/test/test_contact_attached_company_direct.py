...
class TestContactAttachedCompanyDirect:

    def test_should_direct_list_contact_attached_company(self):
        setup = _contact_attached_company_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-contact_attached_company", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["contact_attached_company01"]:
...
def _contact_attached_company_direct_setup(mockres):
