...
class TestDeletedInternalArticleObjectDirect:

    def test_should_direct_list_deleted_internal_article_object(self):
        setup = _deleted_internal_article_object_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-deleted_internal_article_object", "live" if setup["live"] else "unit")
        if _skip:
...
def _deleted_internal_article_object_direct_setup(mockres):
