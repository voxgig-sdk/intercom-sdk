...
describe('DeletedInternalArticleObjectEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    // LIST
    const deleted_internal_article_object_ref01_match = {}

    const deleted_internal_article_object_ref01_list = (await deleted_internal_article_object_ref01_ent.list(deleted_internal_article_object_ref01_match)).map((e) => e.data())

    assert(!isempty(select(deleted_internal_article_object_ref01_list, { id: deleted_internal_article_object_ref01_data.id })))


    // REMOVE
    const deleted_internal_article_object_ref01_match_rm0 = {}
    deleted_internal_article_object_ref01_match_rm0.id = deleted_internal_article_object_ref01_data.id
    await deleted_internal_article_object_ref01_ent.remove(deleted_internal_article_object_ref01_match_rm0)
  

    // LIST
    const deleted_internal_article_object_ref01_match_rt0 = {}

    const deleted_internal_article_object_ref01_list_rt0 = (await deleted_internal_article_object_ref01_ent.list(deleted_internal_article_object_ref01_match_rt0)).map((e) => e.data())
