...
class SubscriptionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set INTERCOM_TEST_SUBSCRIPTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function subscription_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["subscription01", "subscription02", "subscription03", "contact01", "contact02", "contact03"] as $k) {
    ...
    $entid_env_raw = getenv("INTERCOM_TEST_SUBSCRIPTION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["INTERCOM_TEST_SUBSCRIPTION_ENTID"]);
