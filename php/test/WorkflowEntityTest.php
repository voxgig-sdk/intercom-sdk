...
class WorkflowEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Workflow(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = workflow_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "workflow." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set INTERCOM_TEST_WORKFLOW_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $workflow_ref01_data = null;
        if (count($workflow_ref01_data_raw) > 0) {
            $workflow_ref01_data = Helpers::to_map($workflow_ref01_data_raw[0][1]);
        }

        // LOAD
        $workflow_ref01_ent = $client->Workflow(null);
...
function workflow_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/workflow/WorkflowTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["workflow01", "workflow02", "workflow03"] as $k) {
    ...
    $entid_env_raw = getenv("INTERCOM_TEST_WORKFLOW_ENTID");
