...
local CustomObjectInstanceEntity = {}
CustomObjectInstanceEntity.__index = CustomObjectInstanceEntity


function CustomObjectInstanceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CustomObjectInstanceEntity)
  self._name = "custom_object_instance"
  self._client = client
...
function CustomObjectInstanceEntity:get_name()
  return self._name
end


function CustomObjectInstanceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CustomObjectInstanceEntity.new(self._client, opts)
end
...
function CustomObjectInstanceEntity:mark_deleted()
  self._deleted = true
end


function CustomObjectInstanceEntity:deleted()
  return true == self._deleted
end


function CustomObjectInstanceEntity:data_set(args)
  if args ~= nil then
...
function CustomObjectInstanceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CustomObjectInstanceEntity:match_set(args)
  if args ~= nil then
...
function CustomObjectInstanceEntity:match_get()
...
function CustomObjectInstanceEntity:stream(action, args, callopts)
...
---@param reqmatch CustomObjectInstanceLoadMatch
---@param ctrl? table
---@return CustomObjectInstance
---@return string? err
function CustomObjectInstanceEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CustomObjectInstanceCreateData
---@param ctrl? table
---@return CustomObjectInstance
---@return string? err
function CustomObjectInstanceEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch CustomObjectInstanceRemoveMatch
---@param ctrl? table
---@return CustomObjectInstance
---@return string? err
function CustomObjectInstanceEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function CustomObjectInstanceEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CustomObjectInstanceEntity
