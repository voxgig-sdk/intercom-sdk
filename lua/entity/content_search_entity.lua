-- Intercom SDK ContentSearch entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ContentSearchEntity = {}
ContentSearchEntity.__index = ContentSearchEntity


function ContentSearchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ContentSearchEntity)
  self._name = "content_search"
  self._client = client
...
function ContentSearchEntity:get_name()
  return self._name
end


function ContentSearchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ContentSearchEntity.new(self._client, opts)
end
...
function ContentSearchEntity:mark_deleted()
  self._deleted = true
end


function ContentSearchEntity:deleted()
  return true == self._deleted
end


function ContentSearchEntity:data_set(args)
  if args ~= nil then
...
function ContentSearchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ContentSearchEntity:match_set(args)
  if args ~= nil then
...
function ContentSearchEntity:match_get()
...
function ContentSearchEntity:stream(action, args, callopts)
...
---@param reqmatch ContentSearchListMatch
---@param ctrl? table
---@return ContentSearch[]
---@return string? err
function ContentSearchEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ContentSearchEntity:_run_op(ctx, post_done)
  local utility = self._utility
