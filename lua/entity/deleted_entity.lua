-- Intercom SDK Deleted entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DeletedEntity = {}
DeletedEntity.__index = DeletedEntity


function DeletedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DeletedEntity)
  self._name = "deleted"
  self._client = client
...
function DeletedEntity:get_name()
  return self._name
end


function DeletedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DeletedEntity.new(self._client, opts)
end
...
function DeletedEntity:mark_deleted()
  self._deleted = true
end


function DeletedEntity:deleted()
  return true == self._deleted
end


function DeletedEntity:data_set(args)
  if args ~= nil then
...
function DeletedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DeletedEntity:match_set(args)
  if args ~= nil then
...
function DeletedEntity:match_get()
...
function DeletedEntity:stream(action, args, callopts)
...
---@param reqmatch DeletedListMatch
---@param ctrl? table
---@return Deleted[]
---@return string? err
function DeletedEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function DeletedEntity:_run_op(ctx, post_done)
  local utility = self._utility
