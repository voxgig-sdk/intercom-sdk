-- Intercom SDK ActivityLogList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ActivityLogListEntity = {}
ActivityLogListEntity.__index = ActivityLogListEntity


function ActivityLogListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActivityLogListEntity)
  self._name = "activity_log_list"
  self._client = client
...
function ActivityLogListEntity:get_name()
  return self._name
end


function ActivityLogListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActivityLogListEntity.new(self._client, opts)
end
...
function ActivityLogListEntity:mark_deleted()
  self._deleted = true
end


function ActivityLogListEntity:deleted()
  return true == self._deleted
end


function ActivityLogListEntity:data_set(args)
  if args ~= nil then
...
function ActivityLogListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActivityLogListEntity:match_set(args)
  if args ~= nil then
...
function ActivityLogListEntity:match_get()
...
function ActivityLogListEntity:stream(action, args, callopts)
...
---@param reqdata ActivityLogListCreateData
---@param ctrl? table
---@return ActivityLogList
---@return string? err
function ActivityLogListEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ActivityLogListEntity:_run_op(ctx, post_done)
  local utility = self._utility
