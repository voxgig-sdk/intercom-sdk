...
local DataEntity = {}
DataEntity.__index = DataEntity


function DataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DataEntity)
  self._name = "data"
  self._client = client
...
function DataEntity:get_name()
  return self._name
end


function DataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DataEntity.new(self._client, opts)
end
...
function DataEntity:mark_deleted()
  self._deleted = true
end


function DataEntity:deleted()
  return true == self._deleted
end


function DataEntity:data_set(args)
  if args ~= nil then
...
function DataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DataEntity:match_set(args)
  if args ~= nil then
...
function DataEntity:stream(action, args, callopts)
...
---@param reqmatch DataLoadMatch
---@param ctrl? table
---@return Data
---@return string? err
function DataEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata DataCreateData
---@param ctrl? table
---@return Data
---@return string? err
function DataEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function DataEntity:_run_op(ctx, post_done)
  local utility = self._utility
