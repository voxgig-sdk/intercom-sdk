-- Intercom SDK Workflow entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local WorkflowEntity = {}
WorkflowEntity.__index = WorkflowEntity


function WorkflowEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WorkflowEntity)
  self._name = "workflow"
  self._client = client
...
function WorkflowEntity:get_name()
  return self._name
end


function WorkflowEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WorkflowEntity.new(self._client, opts)
end
...
function WorkflowEntity:mark_deleted()
  self._deleted = true
end


function WorkflowEntity:deleted()
  return true == self._deleted
end


function WorkflowEntity:data_set(args)
  if args ~= nil then
...
function WorkflowEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WorkflowEntity:match_set(args)
  if args ~= nil then
...
function WorkflowEntity:match_get()
...
function WorkflowEntity:stream(action, args, callopts)
...
---@param reqmatch WorkflowLoadMatch
---@param ctrl? table
---@return Workflow
---@return string? err
function WorkflowEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WorkflowEntity:_run_op(ctx, post_done)
  local utility = self._utility
