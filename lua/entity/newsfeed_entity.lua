-- Intercom SDK Newsfeed entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local NewsfeedEntity = {}
NewsfeedEntity.__index = NewsfeedEntity


function NewsfeedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NewsfeedEntity)
  self._name = "newsfeed"
  self._client = client
...
function NewsfeedEntity:get_name()
  return self._name
end


function NewsfeedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NewsfeedEntity.new(self._client, opts)
end
...
function NewsfeedEntity:mark_deleted()
  self._deleted = true
end


function NewsfeedEntity:deleted()
  return true == self._deleted
end


function NewsfeedEntity:data_set(args)
  if args ~= nil then
...
function NewsfeedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NewsfeedEntity:match_set(args)
  if args ~= nil then
...
function NewsfeedEntity:match_get()
...
function NewsfeedEntity:stream(action, args, callopts)
...
---@param reqmatch NewsfeedLoadMatch
---@param ctrl? table
---@return Newsfeed
---@return string? err
function NewsfeedEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function NewsfeedEntity:_run_op(ctx, post_done)
  local utility = self._utility
