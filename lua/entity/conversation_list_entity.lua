-- Intercom SDK ConversationList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ConversationListEntity = {}
ConversationListEntity.__index = ConversationListEntity


function ConversationListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ConversationListEntity)
  self._name = "conversation_list"
  self._client = client
...
function ConversationListEntity:get_name()
  return self._name
end


function ConversationListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ConversationListEntity.new(self._client, opts)
end
...
function ConversationListEntity:mark_deleted()
  self._deleted = true
end


function ConversationListEntity:deleted()
  return true == self._deleted
end


function ConversationListEntity:data_set(args)
  if args ~= nil then
...
function ConversationListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ConversationListEntity:match_set(args)
  if args ~= nil then
...
function ConversationListEntity:match_get()
...
function ConversationListEntity:stream(action, args, callopts)
...
---@param reqdata ConversationListCreateData
---@param ctrl? table
---@return ConversationList
---@return string? err
function ConversationListEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ConversationListEntity:_run_op(ctx, post_done)
  local utility = self._utility
