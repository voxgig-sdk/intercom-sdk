...
local ReportingDataExportEntity = {}
ReportingDataExportEntity.__index = ReportingDataExportEntity


function ReportingDataExportEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReportingDataExportEntity)
  self._name = "reporting_data_export"
  self._client = client
...
function ReportingDataExportEntity:get_name()
  return self._name
end


function ReportingDataExportEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReportingDataExportEntity.new(self._client, opts)
end
...
function ReportingDataExportEntity:mark_deleted()
  self._deleted = true
end


function ReportingDataExportEntity:deleted()
  return true == self._deleted
end


function ReportingDataExportEntity:data_set(args)
  if args ~= nil then
...
function ReportingDataExportEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReportingDataExportEntity:match_set(args)
  if args ~= nil then
...
function ReportingDataExportEntity:match_get()
...
function ReportingDataExportEntity:stream(action, args, callopts)
...
---@param reqmatch ReportingDataExportListMatch
---@param ctrl? table
---@return ReportingDataExport[]
---@return string? err
function ReportingDataExportEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata ReportingDataExportCreateData
---@param ctrl? table
---@return ReportingDataExport
---@return string? err
function ReportingDataExportEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function ReportingDataExportEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ReportingDataExportEntity
