-- Intercom SDK ContactSegment entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ContactSegmentEntity = {}
ContactSegmentEntity.__index = ContactSegmentEntity


function ContactSegmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ContactSegmentEntity)
  self._name = "contact_segment"
  self._client = client
...
function ContactSegmentEntity:get_name()
  return self._name
end


function ContactSegmentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ContactSegmentEntity.new(self._client, opts)
end
...
function ContactSegmentEntity:mark_deleted()
  self._deleted = true
end


function ContactSegmentEntity:deleted()
  return true == self._deleted
end


function ContactSegmentEntity:data_set(args)
  if args ~= nil then
...
function ContactSegmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ContactSegmentEntity:match_set(args)
  if args ~= nil then
...
function ContactSegmentEntity:match_get()
...
function ContactSegmentEntity:stream(action, args, callopts)
...
---@param reqmatch ContactSegmentListMatch
---@param ctrl? table
---@return ContactSegment[]
---@return string? err
function ContactSegmentEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ContactSegmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
