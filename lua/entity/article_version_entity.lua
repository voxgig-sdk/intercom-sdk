-- Intercom SDK ArticleVersion entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ArticleVersionEntity = {}
ArticleVersionEntity.__index = ArticleVersionEntity


function ArticleVersionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArticleVersionEntity)
  self._name = "article_version"
  self._client = client
...
function ArticleVersionEntity:get_name()
  return self._name
end


function ArticleVersionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArticleVersionEntity.new(self._client, opts)
end
...
function ArticleVersionEntity:mark_deleted()
  self._deleted = true
end


function ArticleVersionEntity:deleted()
  return true == self._deleted
end


function ArticleVersionEntity:data_set(args)
  if args ~= nil then
...
function ArticleVersionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArticleVersionEntity:match_set(args)
  if args ~= nil then
...
function ArticleVersionEntity:match_get()
...
function ArticleVersionEntity:stream(action, args, callopts)
...
---@param reqmatch ArticleVersionLoadMatch
---@param ctrl? table
---@return ArticleVersion
---@return string? err
function ArticleVersionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ArticleVersionEntity:_run_op(ctx, post_done)
  local utility = self._utility
