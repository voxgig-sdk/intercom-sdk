...
local InternalArticleEntity = {}
InternalArticleEntity.__index = InternalArticleEntity


function InternalArticleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InternalArticleEntity)
  self._name = "internal_article"
  self._client = client
...
function InternalArticleEntity:get_name()
  return self._name
end


function InternalArticleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InternalArticleEntity.new(self._client, opts)
end
...
function InternalArticleEntity:mark_deleted()
  self._deleted = true
end


function InternalArticleEntity:deleted()
  return true == self._deleted
end


function InternalArticleEntity:data_set(args)
  if args ~= nil then
...
function InternalArticleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InternalArticleEntity:match_set(args)
  if args ~= nil then
...
function InternalArticleEntity:match_get()
...
function InternalArticleEntity:stream(action, args, callopts)
...
---@param reqmatch InternalArticleLoadMatch
---@param ctrl? table
---@return InternalArticle
---@return string? err
function InternalArticleEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata InternalArticleUpdateData
---@param ctrl? table
---@return InternalArticle
---@return string? err
function InternalArticleEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function InternalArticleEntity:_run_op(ctx, post_done)
  local utility = self._utility
