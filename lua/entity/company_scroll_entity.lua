-- Intercom SDK CompanyScroll entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CompanyScrollEntity = {}
CompanyScrollEntity.__index = CompanyScrollEntity


function CompanyScrollEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompanyScrollEntity)
  self._name = "company_scroll"
  self._client = client
...
function CompanyScrollEntity:get_name()
  return self._name
end


function CompanyScrollEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompanyScrollEntity.new(self._client, opts)
end
...
function CompanyScrollEntity:mark_deleted()
  self._deleted = true
end


function CompanyScrollEntity:deleted()
  return true == self._deleted
end


function CompanyScrollEntity:data_set(args)
  if args ~= nil then
...
function CompanyScrollEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompanyScrollEntity:match_set(args)
  if args ~= nil then
...
function CompanyScrollEntity:match_get()
...
function CompanyScrollEntity:stream(action, args, callopts)
...
---@param reqmatch CompanyScrollListMatch
---@param ctrl? table
---@return CompanyScroll[]
---@return string? err
function CompanyScrollEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CompanyScrollEntity:_run_op(ctx, post_done)
  local utility = self._utility
