-- Intercom SDK ConversationParticipant entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ConversationParticipantEntity = {}
ConversationParticipantEntity.__index = ConversationParticipantEntity


function ConversationParticipantEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ConversationParticipantEntity)
  self._name = "conversation_participant"
  self._client = client
...
function ConversationParticipantEntity:get_name()
  return self._name
end


function ConversationParticipantEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ConversationParticipantEntity.new(self._client, opts)
end
...
function ConversationParticipantEntity:mark_deleted()
  self._deleted = true
end


function ConversationParticipantEntity:deleted()
  return true == self._deleted
end


function ConversationParticipantEntity:data_set(args)
  if args ~= nil then
...
function ConversationParticipantEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ConversationParticipantEntity:match_set(args)
  if args ~= nil then
...
function ConversationParticipantEntity:match_get()
...
function ConversationParticipantEntity:stream(action, args, callopts)
...
---@param reqdata ConversationParticipantCreateData
---@param ctrl? table
---@return ConversationParticipant
---@return string? err
function ConversationParticipantEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqmatch ConversationParticipantRemoveMatch
---@param ctrl? table
---@return ConversationParticipant
---@return string? err
function ConversationParticipantEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function ConversationParticipantEntity:_run_op(ctx, post_done)
  local utility = self._utility
