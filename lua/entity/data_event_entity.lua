-- Intercom SDK DataEvent entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DataEventEntity = {}
DataEventEntity.__index = DataEventEntity


function DataEventEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DataEventEntity)
  self._name = "data_event"
  self._client = client
...
function DataEventEntity:get_name()
  return self._name
end


function DataEventEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DataEventEntity.new(self._client, opts)
end
...
function DataEventEntity:mark_deleted()
  self._deleted = true
end


function DataEventEntity:deleted()
  return true == self._deleted
end


function DataEventEntity:data_set(args)
  if args ~= nil then
...
function DataEventEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DataEventEntity:match_set(args)
  if args ~= nil then
...
function DataEventEntity:match_get()
...
function DataEventEntity:stream(action, args, callopts)
...
---@param reqdata DataEventCreateData
---@param ctrl? table
---@return DataEvent
---@return string? err
function DataEventEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function DataEventEntity:_run_op(ctx, post_done)
  local utility = self._utility
