-- Intercom SDK AdminWithApp entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AdminWithAppEntity = {}
AdminWithAppEntity.__index = AdminWithAppEntity


function AdminWithAppEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AdminWithAppEntity)
  self._name = "admin_with_app"
  self._client = client
...
function AdminWithAppEntity:get_name()
  return self._name
end


function AdminWithAppEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AdminWithAppEntity.new(self._client, opts)
end
...
function AdminWithAppEntity:mark_deleted()
  self._deleted = true
end


function AdminWithAppEntity:deleted()
  return true == self._deleted
end


function AdminWithAppEntity:data_set(args)
  if args ~= nil then
...
function AdminWithAppEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AdminWithAppEntity:match_set(args)
  if args ~= nil then
...
function AdminWithAppEntity:match_get()
...
function AdminWithAppEntity:stream(action, args, callopts)
...
---@param reqmatch AdminWithAppListMatch
---@param ctrl? table
---@return AdminWithApp[]
---@return string? err
function AdminWithAppEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function AdminWithAppEntity:_run_op(ctx, post_done)
  local utility = self._utility
