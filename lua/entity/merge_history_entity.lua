-- Intercom SDK MergeHistory entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local MergeHistoryEntity = {}
MergeHistoryEntity.__index = MergeHistoryEntity


function MergeHistoryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MergeHistoryEntity)
  self._name = "merge_history"
  self._client = client
...
function MergeHistoryEntity:get_name()
  return self._name
end


function MergeHistoryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MergeHistoryEntity.new(self._client, opts)
end
...
function MergeHistoryEntity:mark_deleted()
  self._deleted = true
end


function MergeHistoryEntity:deleted()
  return true == self._deleted
end


function MergeHistoryEntity:data_set(args)
  if args ~= nil then
...
function MergeHistoryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MergeHistoryEntity:match_set(args)
  if args ~= nil then
...
function MergeHistoryEntity:match_get()
...
function MergeHistoryEntity:stream(action, args, callopts)
...
---@param reqmatch MergeHistoryListMatch
---@param ctrl? table
---@return MergeHistory[]
---@return string? err
function MergeHistoryEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function MergeHistoryEntity:_run_op(ctx, post_done)
  local utility = self._utility
