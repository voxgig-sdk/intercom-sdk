-- Intercom SDK DataEventSummary entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DataEventSummaryEntity = {}
DataEventSummaryEntity.__index = DataEventSummaryEntity


function DataEventSummaryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DataEventSummaryEntity)
  self._name = "data_event_summary"
  self._client = client
...
function DataEventSummaryEntity:get_name()
  return self._name
end


function DataEventSummaryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DataEventSummaryEntity.new(self._client, opts)
end
...
function DataEventSummaryEntity:mark_deleted()
  self._deleted = true
end


function DataEventSummaryEntity:deleted()
  return true == self._deleted
end


function DataEventSummaryEntity:data_set(args)
  if args ~= nil then
...
function DataEventSummaryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DataEventSummaryEntity:match_set(args)
  if args ~= nil then
...
function DataEventSummaryEntity:match_get()
...
function DataEventSummaryEntity:stream(action, args, callopts)
...
---@param reqmatch DataEventSummaryListMatch
---@param ctrl? table
---@return DataEventSummary[]
---@return string? err
function DataEventSummaryEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function DataEventSummaryEntity:_run_op(ctx, post_done)
  local utility = self._utility
