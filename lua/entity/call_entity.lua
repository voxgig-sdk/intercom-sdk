...
local CallEntity = {}
CallEntity.__index = CallEntity


function CallEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CallEntity)
  self._name = "call"
  self._client = client
...
function CallEntity:get_name()
  return self._name
end


function CallEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CallEntity.new(self._client, opts)
end
...
function CallEntity:mark_deleted()
  self._deleted = true
end


function CallEntity:deleted()
  return true == self._deleted
end


function CallEntity:data_set(args)
  if args ~= nil then
...
function CallEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CallEntity:match_set(args)
  if args ~= nil then
...
function CallEntity:stream(action, args, callopts)
...
---@param reqmatch CallLoadMatch
---@param ctrl? table
---@return Call
---@return string? err
function CallEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CallListMatch
---@param ctrl? table
---@return Call[]
---@return string? err
function CallEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata CallCreateData
---@param ctrl? table
---@return Call
---@return string? err
function CallEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function CallEntity:_run_op(ctx, post_done)
  local utility = self._utility
