...
local MacroEntity = {}
MacroEntity.__index = MacroEntity


function MacroEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MacroEntity)
  self._name = "macro"
  self._client = client
...
function MacroEntity:get_name()
  return self._name
end


function MacroEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MacroEntity.new(self._client, opts)
end
...
function MacroEntity:mark_deleted()
  self._deleted = true
end


function MacroEntity:deleted()
  return true == self._deleted
end


function MacroEntity:data_set(args)
  if args ~= nil then
...
function MacroEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MacroEntity:match_set(args)
  if args ~= nil then
...
function MacroEntity:match_get()
...
function MacroEntity:stream(action, args, callopts)
...
---@param reqmatch MacroLoadMatch
---@param ctrl? table
---@return Macro
---@return string? err
function MacroEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch MacroListMatch
---@param ctrl? table
---@return Macro[]
---@return string? err
function MacroEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function MacroEntity:_run_op(ctx, post_done)
  local utility = self._utility
