...
local NewsItemEntity = {}
NewsItemEntity.__index = NewsItemEntity


function NewsItemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NewsItemEntity)
  self._name = "news_item"
  self._client = client
...
function NewsItemEntity:get_name()
  return self._name
end


function NewsItemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NewsItemEntity.new(self._client, opts)
end
...
function NewsItemEntity:mark_deleted()
  self._deleted = true
end


function NewsItemEntity:deleted()
  return true == self._deleted
end


function NewsItemEntity:data_set(args)
  if args ~= nil then
...
function NewsItemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NewsItemEntity:match_set(args)
  if args ~= nil then
...
function NewsItemEntity:match_get()
...
function NewsItemEntity:stream(action, args, callopts)
...
---@param reqmatch NewsItemLoadMatch
---@param ctrl? table
---@return NewsItem
---@return string? err
function NewsItemEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata NewsItemCreateData
---@param ctrl? table
---@return NewsItem
---@return string? err
function NewsItemEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata NewsItemUpdateData
---@param ctrl? table
---@return NewsItem
---@return string? err
function NewsItemEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function NewsItemEntity:_run_op(ctx, post_done)
  local utility = self._utility
