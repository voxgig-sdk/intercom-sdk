-- Intercom SDK CompanyAttachedContact entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CompanyAttachedContactEntity = {}
CompanyAttachedContactEntity.__index = CompanyAttachedContactEntity


function CompanyAttachedContactEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompanyAttachedContactEntity)
  self._name = "company_attached_contact"
  self._client = client
...
function CompanyAttachedContactEntity:get_name()
  return self._name
end


function CompanyAttachedContactEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompanyAttachedContactEntity.new(self._client, opts)
end
...
function CompanyAttachedContactEntity:mark_deleted()
  self._deleted = true
end


function CompanyAttachedContactEntity:deleted()
  return true == self._deleted
end


function CompanyAttachedContactEntity:data_set(args)
  if args ~= nil then
...
function CompanyAttachedContactEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompanyAttachedContactEntity:match_set(args)
  if args ~= nil then
...
function CompanyAttachedContactEntity:match_get()
...
function CompanyAttachedContactEntity:stream(action, args, callopts)
...
---@param reqmatch CompanyAttachedContactListMatch
---@param ctrl? table
---@return CompanyAttachedContact[]
---@return string? err
function CompanyAttachedContactEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CompanyAttachedContactEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CompanyAttachedContactEntity
