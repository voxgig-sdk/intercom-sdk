...
local AdminEntity = {}
AdminEntity.__index = AdminEntity


function AdminEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AdminEntity)
  self._name = "admin"
  self._client = client
...
function AdminEntity:get_name()
  return self._name
end


function AdminEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AdminEntity.new(self._client, opts)
end
...
function AdminEntity:mark_deleted()
  self._deleted = true
end


function AdminEntity:deleted()
  return true == self._deleted
end


function AdminEntity:data_set(args)
  if args ~= nil then
...
function AdminEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AdminEntity:match_set(args)
  if args ~= nil then
...
function AdminEntity:match_get()
...
function AdminEntity:stream(action, args, callopts)
...
---@param reqmatch AdminLoadMatch
---@param ctrl? table
---@return Admin
---@return string? err
function AdminEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch AdminListMatch
---@param ctrl? table
---@return Admin[]
---@return string? err
function AdminEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata AdminUpdateData
---@param ctrl? table
---@return Admin
---@return string? err
function AdminEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function AdminEntity:_run_op(ctx, post_done)
  local utility = self._utility
