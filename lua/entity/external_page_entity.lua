...
local ExternalPageEntity = {}
ExternalPageEntity.__index = ExternalPageEntity


function ExternalPageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ExternalPageEntity)
  self._name = "external_page"
  self._client = client
...
function ExternalPageEntity:get_name()
  return self._name
end


function ExternalPageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ExternalPageEntity.new(self._client, opts)
end
...
function ExternalPageEntity:mark_deleted()
  self._deleted = true
end


function ExternalPageEntity:deleted()
  return true == self._deleted
end


function ExternalPageEntity:data_set(args)
  if args ~= nil then
...
function ExternalPageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ExternalPageEntity:match_set(args)
  if args ~= nil then
...
function ExternalPageEntity:match_get()
...
function ExternalPageEntity:stream(action, args, callopts)
...
---@param reqmatch ExternalPageLoadMatch
---@param ctrl? table
---@return ExternalPage
---@return string? err
function ExternalPageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ExternalPageListMatch
---@param ctrl? table
---@return ExternalPage[]
---@return string? err
function ExternalPageEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata ExternalPageCreateData
---@param ctrl? table
---@return ExternalPage
---@return string? err
function ExternalPageEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata ExternalPageUpdateData
---@param ctrl? table
---@return ExternalPage
---@return string? err
function ExternalPageEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch ExternalPageRemoveMatch
---@param ctrl? table
---@return ExternalPage
---@return string? err
function ExternalPageEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function ExternalPageEntity:_run_op(ctx, post_done)
  local utility = self._utility
