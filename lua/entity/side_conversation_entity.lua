-- Intercom SDK SideConversation entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SideConversationEntity = {}
SideConversationEntity.__index = SideConversationEntity


function SideConversationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SideConversationEntity)
  self._name = "side_conversation"
  self._client = client
...
function SideConversationEntity:get_name()
  return self._name
end


function SideConversationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SideConversationEntity.new(self._client, opts)
end
...
function SideConversationEntity:mark_deleted()
  self._deleted = true
end


function SideConversationEntity:deleted()
  return true == self._deleted
end


function SideConversationEntity:data_set(args)
  if args ~= nil then
...
function SideConversationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SideConversationEntity:match_set(args)
  if args ~= nil then
...
function SideConversationEntity:match_get()
...
function SideConversationEntity:stream(action, args, callopts)
...
---@param reqmatch SideConversationListMatch
---@param ctrl? table
---@return SideConversation[]
---@return string? err
function SideConversationEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function SideConversationEntity:_run_op(ctx, post_done)
  local utility = self._utility
