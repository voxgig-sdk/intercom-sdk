...
local AiCallEntity = {}
AiCallEntity.__index = AiCallEntity


function AiCallEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiCallEntity)
  self._name = "ai_call"
  self._client = client
...
function AiCallEntity:get_name()
  return self._name
end


function AiCallEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiCallEntity.new(self._client, opts)
end
...
function AiCallEntity:mark_deleted()
  self._deleted = true
end


function AiCallEntity:deleted()
  return true == self._deleted
end


function AiCallEntity:data_set(args)
  if args ~= nil then
...
function AiCallEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiCallEntity:match_set(args)
  if args ~= nil then
...
function AiCallEntity:match_get()
...
function AiCallEntity:stream(action, args, callopts)
...
---@param reqmatch AiCallLoadMatch
---@param ctrl? table
---@return AiCall
---@return string? err
function AiCallEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AiCallCreateData
---@param ctrl? table
---@return AiCall
---@return string? err
function AiCallEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function AiCallEntity:_run_op(ctx, post_done)
  local utility = self._utility
