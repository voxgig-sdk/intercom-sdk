-- Intercom SDK CompanyList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CompanyListEntity = {}
CompanyListEntity.__index = CompanyListEntity


function CompanyListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompanyListEntity)
  self._name = "company_list"
  self._client = client
...
function CompanyListEntity:get_name()
  return self._name
end


function CompanyListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompanyListEntity.new(self._client, opts)
end
...
function CompanyListEntity:mark_deleted()
  self._deleted = true
end


function CompanyListEntity:deleted()
  return true == self._deleted
end


function CompanyListEntity:data_set(args)
  if args ~= nil then
...
function CompanyListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompanyListEntity:match_set(args)
  if args ~= nil then
...
function CompanyListEntity:match_get()
...
function CompanyListEntity:stream(action, args, callopts)
...
---@param reqdata CompanyListCreateData
---@param ctrl? table
---@return CompanyList
---@return string? err
function CompanyListEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function CompanyListEntity:_run_op(ctx, post_done)
  local utility = self._utility
