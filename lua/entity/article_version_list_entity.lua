-- Intercom SDK ArticleVersionList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ArticleVersionListEntity = {}
ArticleVersionListEntity.__index = ArticleVersionListEntity


function ArticleVersionListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArticleVersionListEntity)
  self._name = "article_version_list"
  self._client = client
...
function ArticleVersionListEntity:get_name()
  return self._name
end


function ArticleVersionListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArticleVersionListEntity.new(self._client, opts)
end
...
function ArticleVersionListEntity:mark_deleted()
  self._deleted = true
end


function ArticleVersionListEntity:deleted()
  return true == self._deleted
end


function ArticleVersionListEntity:data_set(args)
  if args ~= nil then
...
function ArticleVersionListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArticleVersionListEntity:match_set(args)
  if args ~= nil then
...
function ArticleVersionListEntity:match_get()
...
function ArticleVersionListEntity:stream(action, args, callopts)
...
---@param reqmatch ArticleVersionListListMatch
---@param ctrl? table
---@return ArticleVersionList[]
---@return string? err
function ArticleVersionListEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ArticleVersionListEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ArticleVersionListEntity
