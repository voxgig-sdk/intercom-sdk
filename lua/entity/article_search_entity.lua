-- Intercom SDK ArticleSearch entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ArticleSearchEntity = {}
ArticleSearchEntity.__index = ArticleSearchEntity


function ArticleSearchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArticleSearchEntity)
  self._name = "article_search"
  self._client = client
...
function ArticleSearchEntity:get_name()
  return self._name
end


function ArticleSearchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArticleSearchEntity.new(self._client, opts)
end
...
function ArticleSearchEntity:mark_deleted()
  self._deleted = true
end


function ArticleSearchEntity:deleted()
  return true == self._deleted
end


function ArticleSearchEntity:data_set(args)
  if args ~= nil then
...
function ArticleSearchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArticleSearchEntity:match_set(args)
  if args ~= nil then
...
function ArticleSearchEntity:match_get()
...
function ArticleSearchEntity:stream(action, args, callopts)
...
---@param reqmatch ArticleSearchLoadMatch
---@param ctrl? table
---@return ArticleSearch
---@return string? err
function ArticleSearchEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ArticleSearchEntity:_run_op(ctx, post_done)
  local utility = self._utility
