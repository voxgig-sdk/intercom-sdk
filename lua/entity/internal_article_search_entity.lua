-- Intercom SDK InternalArticleSearch entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local InternalArticleSearchEntity = {}
InternalArticleSearchEntity.__index = InternalArticleSearchEntity


function InternalArticleSearchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InternalArticleSearchEntity)
  self._name = "internal_article_search"
  self._client = client
...
function InternalArticleSearchEntity:get_name()
  return self._name
end


function InternalArticleSearchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InternalArticleSearchEntity.new(self._client, opts)
end
...
function InternalArticleSearchEntity:mark_deleted()
  self._deleted = true
end


function InternalArticleSearchEntity:deleted()
  return true == self._deleted
end


function InternalArticleSearchEntity:data_set(args)
  if args ~= nil then
...
function InternalArticleSearchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InternalArticleSearchEntity:match_set(args)
  if args ~= nil then
...
function InternalArticleSearchEntity:match_get()
...
function InternalArticleSearchEntity:stream(action, args, callopts)
...
---@param reqmatch InternalArticleSearchLoadMatch
---@param ctrl? table
---@return InternalArticleSearch
---@return string? err
function InternalArticleSearchEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function InternalArticleSearchEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return InternalArticleSearchEntity
