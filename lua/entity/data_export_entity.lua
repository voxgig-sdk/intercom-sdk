-- Intercom SDK DataExport entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DataExportEntity = {}
DataExportEntity.__index = DataExportEntity


function DataExportEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DataExportEntity)
  self._name = "data_export"
  self._client = client
...
function DataExportEntity:get_name()
  return self._name
end


function DataExportEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DataExportEntity.new(self._client, opts)
end
...
function DataExportEntity:mark_deleted()
  self._deleted = true
end


function DataExportEntity:deleted()
  return true == self._deleted
end


function DataExportEntity:data_set(args)
  if args ~= nil then
...
function DataExportEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DataExportEntity:match_set(args)
  if args ~= nil then
...
function DataExportEntity:match_get()
...
function DataExportEntity:stream(action, args, callopts)
...
---@param reqdata DataExportCreateData
---@param ctrl? table
---@return DataExport
---@return string? err
function DataExportEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function DataExportEntity:_run_op(ctx, post_done)
  local utility = self._utility
