-- Intercom SDK HandlingEvent entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local HandlingEventEntity = {}
HandlingEventEntity.__index = HandlingEventEntity


function HandlingEventEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HandlingEventEntity)
  self._name = "handling_event"
  self._client = client
...
function HandlingEventEntity:get_name()
  return self._name
end


function HandlingEventEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HandlingEventEntity.new(self._client, opts)
end
...
function HandlingEventEntity:mark_deleted()
  self._deleted = true
end


function HandlingEventEntity:deleted()
  return true == self._deleted
end


function HandlingEventEntity:data_set(args)
  if args ~= nil then
...
function HandlingEventEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HandlingEventEntity:match_set(args)
  if args ~= nil then
...
function HandlingEventEntity:match_get()
...
function HandlingEventEntity:stream(action, args, callopts)
...
---@param reqmatch HandlingEventListMatch
---@param ctrl? table
---@return HandlingEvent[]
---@return string? err
function HandlingEventEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function HandlingEventEntity:_run_op(ctx, post_done)
  local utility = self._utility
