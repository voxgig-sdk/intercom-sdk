-- Intercom SDK WhatsappMessageStatus entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local WhatsappMessageStatusEntity = {}
WhatsappMessageStatusEntity.__index = WhatsappMessageStatusEntity


function WhatsappMessageStatusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WhatsappMessageStatusEntity)
  self._name = "whatsapp_message_status"
  self._client = client
...
function WhatsappMessageStatusEntity:get_name()
  return self._name
end


function WhatsappMessageStatusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WhatsappMessageStatusEntity.new(self._client, opts)
end
...
function WhatsappMessageStatusEntity:mark_deleted()
  self._deleted = true
end


function WhatsappMessageStatusEntity:deleted()
  return true == self._deleted
end


function WhatsappMessageStatusEntity:data_set(args)
  if args ~= nil then
...
function WhatsappMessageStatusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WhatsappMessageStatusEntity:match_set(args)
  if args ~= nil then
...
function WhatsappMessageStatusEntity:match_get()
...
function WhatsappMessageStatusEntity:stream(action, args, callopts)
...
---@param reqmatch WhatsappMessageStatusLoadMatch
---@param ctrl? table
---@return WhatsappMessageStatus
---@return string? err
function WhatsappMessageStatusEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WhatsappMessageStatusEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return WhatsappMessageStatusEntity
