-- Intercom SDK TicketReply entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TicketReplyEntity = {}
TicketReplyEntity.__index = TicketReplyEntity


function TicketReplyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TicketReplyEntity)
  self._name = "ticket_reply"
  self._client = client
...
function TicketReplyEntity:get_name()
  return self._name
end


function TicketReplyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TicketReplyEntity.new(self._client, opts)
end
...
function TicketReplyEntity:mark_deleted()
  self._deleted = true
end


function TicketReplyEntity:deleted()
  return true == self._deleted
end


function TicketReplyEntity:data_set(args)
  if args ~= nil then
...
function TicketReplyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TicketReplyEntity:match_set(args)
  if args ~= nil then
...
function TicketReplyEntity:match_get()
...
function TicketReplyEntity:stream(action, args, callopts)
...
---@param reqdata TicketReplyCreateData
---@param ctrl? table
---@return TicketReply
---@return string? err
function TicketReplyEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function TicketReplyEntity:_run_op(ctx, post_done)
  local utility = self._utility
