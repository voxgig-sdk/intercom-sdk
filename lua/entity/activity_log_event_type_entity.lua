-- Intercom SDK ActivityLogEventType entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ActivityLogEventTypeEntity = {}
ActivityLogEventTypeEntity.__index = ActivityLogEventTypeEntity


function ActivityLogEventTypeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActivityLogEventTypeEntity)
  self._name = "activity_log_event_type"
  self._client = client
...
function ActivityLogEventTypeEntity:get_name()
  return self._name
end


function ActivityLogEventTypeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActivityLogEventTypeEntity.new(self._client, opts)
end
...
function ActivityLogEventTypeEntity:mark_deleted()
  self._deleted = true
end


function ActivityLogEventTypeEntity:deleted()
  return true == self._deleted
end


function ActivityLogEventTypeEntity:data_set(args)
  if args ~= nil then
...
function ActivityLogEventTypeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActivityLogEventTypeEntity:match_set(args)
  if args ~= nil then
...
function ActivityLogEventTypeEntity:match_get()
...
function ActivityLogEventTypeEntity:stream(action, args, callopts)
...
---@param reqmatch ActivityLogEventTypeListMatch
---@param ctrl? table
---@return ActivityLogEventType[]
---@return string? err
function ActivityLogEventTypeEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ActivityLogEventTypeEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ActivityLogEventTypeEntity
