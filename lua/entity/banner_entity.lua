-- Intercom SDK Banner entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local BannerEntity = {}
BannerEntity.__index = BannerEntity


function BannerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BannerEntity)
  self._name = "banner"
  self._client = client
...
function BannerEntity:get_name()
  return self._name
end


function BannerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BannerEntity.new(self._client, opts)
end
...
function BannerEntity:mark_deleted()
  self._deleted = true
end


function BannerEntity:deleted()
  return true == self._deleted
end


function BannerEntity:data_set(args)
  if args ~= nil then
...
function BannerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BannerEntity:match_set(args)
  if args ~= nil then
...
function BannerEntity:match_get()
...
function BannerEntity:stream(action, args, callopts)
...
---@param reqmatch BannerListMatch
---@param ctrl? table
---@return Banner[]
---@return string? err
function BannerEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function BannerEntity:_run_op(ctx, post_done)
  local utility = self._utility
