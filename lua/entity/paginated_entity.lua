-- Intercom SDK Paginated entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PaginatedEntity = {}
PaginatedEntity.__index = PaginatedEntity


function PaginatedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PaginatedEntity)
  self._name = "paginated"
  self._client = client
...
function PaginatedEntity:get_name()
  return self._name
end


function PaginatedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PaginatedEntity.new(self._client, opts)
end
...
function PaginatedEntity:mark_deleted()
  self._deleted = true
end


function PaginatedEntity:deleted()
  return true == self._deleted
end


function PaginatedEntity:data_set(args)
  if args ~= nil then
...
function PaginatedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PaginatedEntity:match_set(args)
  if args ~= nil then
...
function PaginatedEntity:match_get()
...
function PaginatedEntity:stream(action, args, callopts)
...
---@param reqmatch PaginatedListMatch
---@param ctrl? table
---@return Paginated[]
---@return string? err
function PaginatedEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PaginatedEntity:_run_op(ctx, post_done)
  local utility = self._utility
