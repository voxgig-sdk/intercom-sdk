-- Intercom SDK ContactList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ContactListEntity = {}
ContactListEntity.__index = ContactListEntity


function ContactListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ContactListEntity)
  self._name = "contact_list"
  self._client = client
...
function ContactListEntity:get_name()
  return self._name
end


function ContactListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ContactListEntity.new(self._client, opts)
end
...
function ContactListEntity:mark_deleted()
  self._deleted = true
end


function ContactListEntity:deleted()
  return true == self._deleted
end


function ContactListEntity:data_set(args)
  if args ~= nil then
...
function ContactListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ContactListEntity:match_set(args)
  if args ~= nil then
...
function ContactListEntity:match_get()
...
function ContactListEntity:stream(action, args, callopts)
...
---@param reqdata ContactListCreateData
---@param ctrl? table
---@return ContactList
---@return string? err
function ContactListEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ContactListEntity:_run_op(ctx, post_done)
  local utility = self._utility
