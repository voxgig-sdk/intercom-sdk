-- Intercom SDK CompanyAttachedSegment entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CompanyAttachedSegmentEntity = {}
CompanyAttachedSegmentEntity.__index = CompanyAttachedSegmentEntity


function CompanyAttachedSegmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompanyAttachedSegmentEntity)
  self._name = "company_attached_segment"
  self._client = client
...
function CompanyAttachedSegmentEntity:get_name()
  return self._name
end


function CompanyAttachedSegmentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompanyAttachedSegmentEntity.new(self._client, opts)
end
...
function CompanyAttachedSegmentEntity:mark_deleted()
  self._deleted = true
end


function CompanyAttachedSegmentEntity:deleted()
  return true == self._deleted
end


function CompanyAttachedSegmentEntity:data_set(args)
  if args ~= nil then
...
function CompanyAttachedSegmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompanyAttachedSegmentEntity:match_set(args)
  if args ~= nil then
...
function CompanyAttachedSegmentEntity:match_get()
...
function CompanyAttachedSegmentEntity:stream(action, args, callopts)
...
---@param reqmatch CompanyAttachedSegmentListMatch
---@param ctrl? table
---@return CompanyAttachedSegment[]
---@return string? err
function CompanyAttachedSegmentEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CompanyAttachedSegmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CompanyAttachedSegmentEntity
