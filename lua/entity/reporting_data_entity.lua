-- Intercom SDK ReportingData entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ReportingDataEntity = {}
ReportingDataEntity.__index = ReportingDataEntity


function ReportingDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReportingDataEntity)
  self._name = "reporting_data"
  self._client = client
...
function ReportingDataEntity:get_name()
  return self._name
end


function ReportingDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReportingDataEntity.new(self._client, opts)
end
...
function ReportingDataEntity:mark_deleted()
  self._deleted = true
end


function ReportingDataEntity:deleted()
  return true == self._deleted
end


function ReportingDataEntity:data_set(args)
  if args ~= nil then
...
function ReportingDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReportingDataEntity:match_set(args)
  if args ~= nil then
...
function ReportingDataEntity:match_get()
...
function ReportingDataEntity:stream(action, args, callopts)
...
---@param reqmatch ReportingDataLoadMatch
---@param ctrl? table
---@return ReportingData
---@return string? err
function ReportingDataEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ReportingDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
