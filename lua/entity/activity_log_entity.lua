-- Intercom SDK ActivityLog entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ActivityLogEntity = {}
ActivityLogEntity.__index = ActivityLogEntity


function ActivityLogEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActivityLogEntity)
  self._name = "activity_log"
  self._client = client
...
function ActivityLogEntity:get_name()
  return self._name
end


function ActivityLogEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActivityLogEntity.new(self._client, opts)
end
...
function ActivityLogEntity:mark_deleted()
  self._deleted = true
end


function ActivityLogEntity:deleted()
  return true == self._deleted
end


function ActivityLogEntity:data_set(args)
  if args ~= nil then
...
function ActivityLogEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActivityLogEntity:match_set(args)
  if args ~= nil then
...
function ActivityLogEntity:match_get()
...
function ActivityLogEntity:stream(action, args, callopts)
...
---@param reqmatch ActivityLogListMatch
---@param ctrl? table
---@return ActivityLog[]
---@return string? err
function ActivityLogEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ActivityLogEntity:_run_op(ctx, post_done)
  local utility = self._utility
