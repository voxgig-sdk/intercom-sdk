-- Intercom SDK TicketState entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TicketStateEntity = {}
TicketStateEntity.__index = TicketStateEntity


function TicketStateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TicketStateEntity)
  self._name = "ticket_state"
  self._client = client
...
function TicketStateEntity:get_name()
  return self._name
end


function TicketStateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TicketStateEntity.new(self._client, opts)
end
...
function TicketStateEntity:mark_deleted()
  self._deleted = true
end


function TicketStateEntity:deleted()
  return true == self._deleted
end


function TicketStateEntity:data_set(args)
  if args ~= nil then
...
function TicketStateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TicketStateEntity:match_set(args)
  if args ~= nil then
...
function TicketStateEntity:match_get()
...
function TicketStateEntity:stream(action, args, callopts)
...
---@param reqmatch TicketStateListMatch
---@param ctrl? table
---@return TicketState[]
---@return string? err
function TicketStateEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function TicketStateEntity:_run_op(ctx, post_done)
  local utility = self._utility
