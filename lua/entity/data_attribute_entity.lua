...
local DataAttributeEntity = {}
DataAttributeEntity.__index = DataAttributeEntity


function DataAttributeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DataAttributeEntity)
  self._name = "data_attribute"
  self._client = client
...
function DataAttributeEntity:get_name()
  return self._name
end


function DataAttributeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DataAttributeEntity.new(self._client, opts)
end
...
function DataAttributeEntity:mark_deleted()
  self._deleted = true
end


function DataAttributeEntity:deleted()
  return true == self._deleted
end


function DataAttributeEntity:data_set(args)
  if args ~= nil then
...
function DataAttributeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DataAttributeEntity:match_set(args)
  if args ~= nil then
...
function DataAttributeEntity:match_get()
...
function DataAttributeEntity:stream(action, args, callopts)
...
---@param reqmatch DataAttributeListMatch
---@param ctrl? table
---@return DataAttribute[]
---@return string? err
function DataAttributeEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata DataAttributeCreateData
---@param ctrl? table
---@return DataAttribute
---@return string? err
function DataAttributeEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata DataAttributeUpdateData
---@param ctrl? table
---@return DataAttribute
---@return string? err
function DataAttributeEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function DataAttributeEntity:_run_op(ctx, post_done)
  local utility = self._utility
