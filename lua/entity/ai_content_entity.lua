-- Intercom SDK AiContent entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AiContentEntity = {}
AiContentEntity.__index = AiContentEntity


function AiContentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiContentEntity)
  self._name = "ai_content"
  self._client = client
...
function AiContentEntity:get_name()
  return self._name
end


function AiContentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiContentEntity.new(self._client, opts)
end
...
function AiContentEntity:mark_deleted()
  self._deleted = true
end


function AiContentEntity:deleted()
  return true == self._deleted
end


function AiContentEntity:data_set(args)
  if args ~= nil then
...
function AiContentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiContentEntity:match_set(args)
  if args ~= nil then
...
function AiContentEntity:match_get()
...
function AiContentEntity:stream(action, args, callopts)
...
---@param reqmatch AiContentRemoveMatch
---@param ctrl? table
---@return AiContent
---@return string? err
function AiContentEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function AiContentEntity:_run_op(ctx, post_done)
  local utility = self._utility
