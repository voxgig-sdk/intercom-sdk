-- Intercom SDK FinAgent entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local FinAgentEntity = {}
FinAgentEntity.__index = FinAgentEntity


function FinAgentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FinAgentEntity)
  self._name = "fin_agent"
  self._client = client
...
function FinAgentEntity:get_name()
  return self._name
end


function FinAgentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FinAgentEntity.new(self._client, opts)
end
...
function FinAgentEntity:mark_deleted()
  self._deleted = true
end


function FinAgentEntity:deleted()
  return true == self._deleted
end


function FinAgentEntity:data_set(args)
  if args ~= nil then
...
function FinAgentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FinAgentEntity:match_set(args)
  if args ~= nil then
...
function FinAgentEntity:match_get()
...
function FinAgentEntity:stream(action, args, callopts)
...
---@param reqdata FinAgentCreateData
---@param ctrl? table
---@return FinAgent
---@return string? err
function FinAgentEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function FinAgentEntity:_run_op(ctx, post_done)
  local utility = self._utility
