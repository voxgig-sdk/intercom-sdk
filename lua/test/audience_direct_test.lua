...
describe("AudienceDirect", function()
  it("should direct-list-audience", function()
    local setup = audience_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-audience", setup.live and "live" or "unit")
  ...
  it("should direct-load-audience", function()
    local setup = audience_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-audience", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function audience_direct_setup(mockres)
