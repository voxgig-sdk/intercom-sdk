...
describe("ContentSnippetDirect", function()
  it("should direct-list-content_snippet", function()
    local setup = content_snippet_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-content_snippet", setup.live and "live" or "unit")
  ...
  it("should direct-load-content_snippet", function()
    local setup = content_snippet_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-content_snippet", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function content_snippet_direct_setup(mockres)
