...
describe("TeamMetricListDirect", function()
  it("should direct-list-team_metric_list", function()
    local setup = team_metric_list_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-team_metric_list", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"team_metric_list01"}) do
...
function team_metric_list_direct_setup(mockres)
