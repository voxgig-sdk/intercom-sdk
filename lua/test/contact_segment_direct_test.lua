...
describe("ContactSegmentDirect", function()
  it("should direct-list-contact_segment", function()
    local setup = contact_segment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-contact_segment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"contact_segment01"}) do
...
function contact_segment_direct_setup(mockres)
