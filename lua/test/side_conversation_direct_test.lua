...
describe("SideConversationDirect", function()
  it("should direct-list-side_conversation", function()
    local setup = side_conversation_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-side_conversation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"conversation01"}) do
...
function side_conversation_direct_setup(mockres)
