-- CompanyAttachedSegment direct test
...
describe("CompanyAttachedSegmentDirect", function()
  it("should direct-list-company_attached_segment", function()
    local setup = company_attached_segment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-company_attached_segment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"company_attached_segment01"}) do
...
function company_attached_segment_direct_setup(mockres)
