...
describe("AdminDirect", function()
  it("should direct-list-admin", function()
    local setup = admin_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-admin", setup.live and "live" or "unit")
  ...
  it("should direct-load-admin", function()
    local setup = admin_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-admin", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function admin_direct_setup(mockres)
