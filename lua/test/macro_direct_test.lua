...
describe("MacroDirect", function()
  it("should direct-list-macro", function()
    local setup = macro_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-macro", setup.live and "live" or "unit")
  ...
  it("should direct-load-macro", function()
    local setup = macro_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-macro", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function macro_direct_setup(mockres)
