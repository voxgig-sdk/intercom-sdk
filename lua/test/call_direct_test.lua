...
describe("CallDirect", function()
  it("should direct-list-call", function()
    local setup = call_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-call", setup.live and "live" or "unit")
  ...
  it("should direct-load-call", function()
    local setup = call_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-call", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function call_direct_setup(mockres)
