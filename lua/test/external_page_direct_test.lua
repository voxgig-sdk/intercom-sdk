...
describe("ExternalPageDirect", function()
  it("should direct-list-external_page", function()
    local setup = external_page_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-external_page", setup.live and "live" or "unit")
  ...
  it("should direct-load-external_page", function()
    local setup = external_page_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-external_page", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function external_page_direct_setup(mockres)
