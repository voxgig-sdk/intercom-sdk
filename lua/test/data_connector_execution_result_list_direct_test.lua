-- DataConnectorExecutionResultList direct test
...
describe("DataConnectorExecutionResultListDirect", function()
  it("should direct-list-data_connector_execution_result_list", function()
    local setup = data_connector_execution_result_list_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-data_connector_execution_result_list", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"data_connector_execution_result_list01"}) do
...
function data_connector_execution_result_list_direct_setup(mockres)
