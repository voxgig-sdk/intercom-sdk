...
//
// NewIntercomSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.IntercomSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewIntercomSDK(opts)
}
...
func fcCandidates(client *sdk.IntercomSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.IntercomSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.IntercomSDK, name string) any {
